                        ...
                        <v-table :data="calons" :filters="filters" :currentPage.sync="currentPage" :pageSize="7"
                            ...
                            <tbody slot="body" slot-scope="{displayData}">
                                <tr v-for="(row, index) in displayData" :key="row.id">
                                    ...
                                    <td>{{ row.siap }}</td>
                                    <td>{{ (row.hari == '' ? 'belum' : row.hari + ', ' + row.tanggal) }}</td>
                    ...
                    <div class="modal-content">
                        <form @submit.prevent="editmode ? updateData() : createData()">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="form-group row">
                                    <label class="col-md-4 col-form-label">Kesiapan Seragam :</label>
                                    <div class="col-md-8">
    ...
    export default {
        ...
        methods: {
            ...
            updateData() {
                this.$Progress.start();
                this.form
                    .put("../api/suratseragam/" + this.form.id)
                    .then(() => {
                        $("#addModal").modal("hide");
                        Fire.$emit("listData");
                        Toast.fire({
                        type: "success",
                        title: "Data Surat Pengambilan Seragam"
                        });
                        this.$Progress.finish();
                    })
                    .catch(() => {
            ...
            editModal(suratseragam) {
                this.editmode = true;
                this.form.reset();
                $("#addModal").modal("show");
                this.form.fill(suratseragam);
            },
