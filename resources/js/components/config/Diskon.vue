            ...
            <div class="col-md-8">
                <div class="card border-info">
                    <div class="card-header bg-info">
                        <h3 class="card-title">Daftar Jenis Diskon</h3>
                    ...
                    <div class="card-body p-0">
                        <v-table :data="diskons" :filters="filters" :currentPage.sync="currentPage" :pageSize="7"
                            @totalPagesChanged="totalPages = $event"
                            class="table table-bordered table-hover table-head-fixed">
                            <thead slot="head">
                                <th>No.</th>
                                <v-th sortKey="jenis">Jenis Diskon</v-th>
                                <v-th sortKey="tp_id">Tahun Ajaran</v-th>
                            ...
                            <tbody slot="body" slot-scope="{displayData}">
                                <tr v-for="(row, index) in displayData" :key="row.id">
                                    ...
                                    <td class="text-center">{{ row.berlaku_sampai }}</td>
                    ...
                    <div class="modal-content">
                        <form @submit.prevent="editmode ? updateData() : createData()">
                            <div class="modal-header">
                                <h5 class="modal-title" v-show="!editmode" id="addModalLabel">Form Tambah Diskon</h5>
                                <h5 class="modal-title" v-show="editmode" id="addModalLabel">Form Edit Diskon</h5>
                            ...
                            <div class="modal-body">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-8">
                                        <select v-model="form.tp_id" name="tp_id" class="form-control" id="tp_id">
                                            <option v-for="tps in category" :key="tps.id"
                                                v-bind:value="tps.name">{{ tps.name }}</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row">
                                ...
                                <div class="form-group row">
                                    <label class="col-sm-4 col-form-label">Nama Diskon</label>
                                    <div class="col-sm-8">
                                        <input v-model="form.nama" type="text" name="nama" class="form-control"
                                            :class="{ 'is-invalid':form.errors.has('nama') }" id="nama"
