            ...
            <div class="col-md-12">
                <div class="card border-info">
                    <div class="card-header bg-info">
                        <h3 class="card-title">Daftar Nama Pewawancara</h3>
                        ...
                        <v-table :data="datas" :filters="filters" :currentPage.sync="currentPage" :pageSize="7"
                            ...
                            <tbody slot="body" slot-scope="{displayData}">
                                <tr v-for="(row, index) in displayData" :key="row.id">
                                    ...
                                    <td>{{ row.nama }}</td>
                                    <td style="text-align: center;">{{ row.unitnya.name }}</td>
                    ...
                    <div class="modal-content">
                        <form @submit.prevent="editmode ? updateData() : createData()">
                            <div class="modal-header">
                                <h5 class="modal-title" v-show="!editmode" id="addModalLabel">Form Tambah Pewawancara</h5>
                                <h5 class="modal-title" v-show="editmode" id="addModalLabel">Form Edit Pewawancara</h5>
                            ...
                            <div class="modal-body">
                                <div class="form-group row">
                                    <label class="col-sm-4 col-form-label">Nama Pewawancara :</label>
                                    <div class="col-sm-8">
                                        <v-select v-model="form.nip" :options=pegawais></v-select>
...
<script>
    export default {
        ...
        methods: {
            listData() {
                this.$Progress.start();
                axios.get("../api/pewawancaras").then(({data}) => (this.datas = data));
                axios.get("../api/units").then(({ data }) => (this.units = data));
                axios.get("../sdmsmart").then(({ data }) => (this.pegawais = data));
            ...
            createData() {
                this.$Progress.start();
                this.form
                    .post("../api/pewawancaras")
                    .then(() => {
                        $("#addModal").modal("hide");
                        Fire.$emit("listData");
                        Toast.fire({
                            type: "success",
                            title: "Tambah Data Pewawancara Berhasil"
                        });
                        this.$Progress.finish();
                    })
                    .catch(() => {
            ...
            updateData() {
                this.$Progress.start();
                this.form
                    .put("../api/pewawancaras/" + this.form.id)
                    .then(() => {
                        $("#addModal").modal("hide");
                        Fire.$emit("listData");
                        Toast.fire({
                            type: "success",
                            title: "Berhasil Update Pewawancara"
                        });
                        this.$Progress.finish();
                    })
                    .catch(() => {
            ...
            deleteData(id) {
                Swal.fire({
                    ...
                    if (result.value) {
                        this.form
                            .delete("../api/pewawancaras/" + id)
                            .then(() => {
                                Swal.fire("Berhasil!", "Pewawancara telah di hapus.", "success");
                                Fire.$emit("listData");
                            })
                            .catch(() => {
                                Swal.fire(
