<!-- unit tk -->

    <h3>TKIT Nurul Fikri</h3>
    <div class="row">
        <div class="col-md-6">
            <table class="table table-bordered">
                <thead>
                    <tr>
                        <th colspan="4" class="text-center">TK A</th>
                    </tr>
                    <tr>
                        <th class="text-center">Komponen Biaya PPDB</th>
                        {{-- <th class="text-center">Reguler 2</th> --}}
                        {{-- <th class="text-center">Reguler 3</th> --}}
                        <th class="text-center">Nominal</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>Dana Pengembangan</td>
                        {{-- <td class="currency">Rp. 8.000.000,-</td> --}}
                        <td class="currency">Rp. 8.500.000,-</td>
                    </tr>
                    <tr>
                        <td>Dana Pendidikan</td>
                        {{-- <td class="currency">Rp. 8.500.000,-</td> --}}
                        <td class="currency">Rp. 8.500.000,-</td>
                    </tr>
                    <tr>
                        <td>Iuran SPP Bulan Juli</td>
                        {{-- <td class="currency">Rp. 1.200.000,-</td> --}}
                        <td class="currency">Rp. 1.250.000,-</td>
                    </tr>
                    <tr>
                        <td>Iuran Komite Sekolah/tahun</td>
                        {{-- <td class="currency">Rp. 300.000,-</td> --}}
                        <td class="currency">Rp. 300.000,-</td>
                    </tr>
                    <tr>
                        <td>Biaya Seragam PUTRA</td>
                        {{-- <td class="currency">Rp. 1.200.000,-</td> --}}
                        <td class="currency">Rp. 1.200.000,-</td>
                    </tr>
                    <tr>
                        <td>Biaya Seragam PUTRI</td>
                        {{-- <td class="currency">Rp. 1.400.000,-</td> --}}
                        <td class="currency">Rp. 1.400.000,-</td>
                    </tr>
                    <tr bgcolor="#01DFD7">
                        <td><b>Total Biaya PUTRA</b></td>
                        {{-- <td class="currency"><b>Rp. 19.200.000,-</b></td> --}}
                        <td class="currency"><b>Rp. 19.750.000,-</b></td>
                    </tr>
                    <tr bgcolor="#f1948a">
                        <td><b>Total Biaya PUTRI</b></td>
                        {{-- <td class="currency"><b>Rp. 19.400.000,-</b></td> --}}
                        <td class="currency"><b>Rp. 19.950.000,-</b></td>
                    </tr>
                    <tr>
                        {{-- <th colspan="4"> --}}
                        <th colspan="3">
                            <ul>
                                <li>Dana Daftar Ulang Tahunan (Kegiatan Dan buku paket) mulai tahun kedua sebesar Rp. 2.000.000 / tahun</li>
                                <li>Prediksi Masa orientasi siswa baru pada tanggal 5 – 10 Juli 2021</li>
                                <li>Prediksi Siswa mulai masuk tanggal 13 Juli 2021</li>
                            </ul>
                        </th>
                    </tr>
                </tbody>
            </table>
        </div>
        <div class="col-md-6">
            <table class="table table-bordered">
            <thead>
                    <tr>
                        <th colspan="4" class="text-center">TK B</th>
                    </tr>
                    <tr>
                        <th class="text-center">Komponen Biaya PSB</th>
                        {{-- <th class="text-center">Reguler 2</th> --}}
                        {{-- <th class="text-center">Reguler 3</th> --}}
                        <th class="text-center">Nominal</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>Dana Pengembangan</td>
                        {{-- <td class="currency">Rp. 5.000.000,-</td> --}}
                        <td class="currency">Rp. 6.000.000,-</td>
                    </tr>
                    <tr>
                        <td>Dana Pendidikan</td>
                        {{-- <td class="currency">Rp. 8.000.000,-</td> --}}
                        <td class="currency">Rp. 8.000.000,-</td>
                    </tr>
                    <tr>
                        <td>Iuran SPP Bulan Juli</td>
                        {{-- <td class="currency">Rp. 1.200.000,-</td> --}}
                        <td class="currency">Rp. 1.250.000,-</td>
                    </tr>
                    <tr>
                        <td>Iuran Komite Sekolah/tahun</td>
                        {{-- <td class="currency">Rp. 300.000,-</td> --}}
                        <td class="currency">Rp. 300.000,-</td>
                    </tr>
                    <tr>
                        <td>Biaya Seragam PUTRA</td>
                        {{-- <td class="currency">Rp. 1.200.000,-</td> --}}
                        <td class="currency">Rp. 1.200.000,-</td>
                    </tr>
                    <tr>
                        <td>Biaya Seragam PUTRI</td>
                        {{-- <td class="currency">Rp. 1.400.000,-</td> --}}
                        <td class="currency">Rp. 1.400.000,-</td>
                    </tr>
                    <tr bgcolor="#01DFD7">
                        <td><b>Total Biaya PUTRA</b></td>
                        {{-- <td class="currency"><b>Rp. 15.700.000,-</b></td> --}}
                        <td class="currency"><b>Rp. 16.750.000,-</b></td>
                    </tr>
                    <tr bgcolor="#f1948a">
                        <td><b>Total Biaya PUTRI</b></td>
                        {{-- <td class="currency"><b>Rp. 15.900.000,-</b></td> --}}
                        <td class="currency"><b>Rp. 16.950.000,-</b></td>
                    </tr>
                    <tr>
                        {{-- <th colspan="4"> --}}
                        <th colspan="3">
                            <ul>
                                <li>Dana Daftar Ulang Tahunan (Kegiatan Dan buku paket) mulai tahun kedua sebesar Rp. 2.000.000 / tahun</li>
                                <li>Prediksi Masa orientasi siswa baru pada tanggal 5 – 10 Juli 2021</li>
                                <li>Prediksi Siswa mulai masuk tanggal 13 Juli 2021</li>
                            </ul>
                        </th>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
