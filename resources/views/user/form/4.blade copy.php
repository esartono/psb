<div class="text-right">
    ...
    <form role="form" method="POST" action="{{ route('add.calon') }}">
        ...
        <div class="form-group row">
            ...
            <div class="col-md-4">
                <input type="text" name="name" class="form-control" id="name" placeholder="Nama Lengkap" {{ $calon->asal_nf == 1 ? "readonly" : "" }} value="{{ $calon->name == '-' ? '' : $calon->name }}" required>
            </div>
            <label class="col-md-2 col-form-label required">Panggilan</label>
            <div class="col-md-3">
                <input type="text" name="panggilan" class="form-control" id="panggilan" placeholder="Nama Panggilan" value="{{ $calon->panggilan == '-' ? '' : $calon->panggilan }}" required>
            </div>
        </div>
        <div class="form-group row">
            <label class="col-md-3 col-form-label required">Tempat Lahir</label>
            <div class="col-md-4">
                <input type="text" name="tempat_lahir" class="form-control" id="tempat_lahir" placeholder="Tempat Lahir" value="{{ $calon->tempat_lahir == null ? '' : $calon->tempat_lahir }}" required>
            </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-md-4">
                <input type="text" name="nisn" class="form-control" id="nisn" placeholder="NISN" value="{{ is_null($calon->nisn) ? '' : $calon->nisn }}" required onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="10" minlength="10">
            </div>
        </div>
        @endif
        <div class="form-group row">
            ...
            <div class="col-md-4">
                <input type="text" name="nik" class="form-control" id="nik" placeholder="Nomor Induk Kependudukan" value="{{ $calon->nik == '-' ? '' : $calon->nik }}" required onkeypress="return event.charCode >= 48 && event.charCode <= 57" maxlength="16" minlength="16">
            </div>
        </div>
        <div class="form-group row">
            <label class="col-md-3 col-form-label">Rencana Masuk Sekolah</label>
            <div class="col-md-4">
                <select name="rencana_masuk" class="form-control" id="rencana_masuk" required>
                    @if($calon->pindahan > 0)
                    <option value="" selected disabled>Pilih rencana masuk sekolah</option>
                    ...
                    @endif
                    <option value="7" {{ $calon->pindahan > 0 ? "" : "selected" }}>Juli - {{ (int)substr(Auth::user()->tp_name, 0, 4) }}</option>
                </select>
            </div>
        </div>
        <div class="form-group row">
