@isset($calon->bayarppdb)
    @if($calon->bayarppdb['cpsb']->lunas == 0)
        ...
        <div class="callout callout-success">
            ...
            <p>Input Ukuran Seragam mulai tanggal 14 Januari {{ (int)taAktif() }}</p>
        </div>
        <hr>
    @endif
    @if($calon->bayarppdb['cpsb']->lunas == 1)
        ...
        <hr>
        @if((int)date('m') >= 1 && (int)date('m') < 9)
            @if((int)date('m') == 1 && (int)date('d') < 14)
                <div class="callout callout-success">
                    <h5 class="mb-3">Input Ukuran Seragam</h5>
                    <p>Input Ukuran Seragam mulai tanggal 14 Januari {{ (int)taAktif() }}</p>
                    <a href="/file/Informasi Input Ukuran Seragam Peserta Didik baru 25-26.pdf" class="btn btn-info" style="color: white; text-decoration: none;" target="_blank">Surat Edaran Pengukuran Seragam</a>
                </div>
                <hr>    
            @else
                <div class="callout callout-success">
