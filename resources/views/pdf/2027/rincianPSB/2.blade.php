...
Lampiran 2 : <b>Pembiayaan Reguler 2</b>
...
<h5>A. Pembiayaan Peserta Didik Baru TA {{ $tp_awal }} - {{ $tp_akhir }}</h5>
    <table class="rincian">
        ...
        <tr>
          <td width="5%"> {{ $k+1 }} </td>
          <td width="50%"> {{ $b }} </td>
          <td align="right"> {{ number_format($biaya2[$b]) }} </td>
        </tr>
        @endforeach
        <tr>
          <td width="5%"> 5 </td>
        ...
        <tr>
          ...
          <td align="right"> <strong>{{ number_format($total2+$ctg->infaq+$ctg->infaqnfpeduli) }}</strong> </td>
        </tr>
      </table>
      <br>
      <table>
        <tr>
          ...
          SIT Nurul Fikri<br><h3><strong>Rp. {{ number_format($totalAll[2]+$total2+$ctg->infaq+$ctg->infaqnfpeduli) }}</strong></h3></td>
        </tr>
      </table>
</td>
<td valign="top" >
...
@foreach($kelass as $k=>$b)
        ...
        <tr>
          <td width="75%">{{ $kelas[$b->name]['ket2'] }} @ Rp. {{ number_format($kelas[$b->name]['spp2']) }} <br></td>
          <td align="right">Rp. {{ number_format($kelas[$b->name]['total2']) }}</td>
        </tr>
        @if(isset($kelas[$b->name]['daul2']))
        <tr>
          <td> Dana Tahunan</td>
          <td align="right">Rp. {{ number_format($kelas[$b->name]['daul2']) }}</td>
        </tr>
        @endif
@endforeach
</table>
