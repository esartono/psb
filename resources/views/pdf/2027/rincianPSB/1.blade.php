  ...
  <tr>
    <td valign="top" width="47%">
      ...
      <table class="rincian">
        ...
        @endforeach
        {{-- <tr>
          <td width="5%"> {{ $nextk+1 }} </td>
          <td> {{ $ketProgramNF }} </td>
          <td align="right"> {{ number_format($biayaProgramNF) }} </td>
        </tr> --}}
        <tr>
          <td width="5%"> {{ $nextk+1 }} </td>
    ...
    <td valign="top" >
      ...
      <table class="global">
        ...
        {{-- <tr>
          <td><b>{{ $ketProgramNF }}</b></td>
          <td align="right"> <strong>Rp. {{ number_format($biayaProgramNF) }}</strong> </td>
        </tr> --}}
...
@if($calon->rencana_masuk == 7)
  <p style="font-size: 70%; font-weight: bold">
    <i>*Pembayaran Biaya Daftar Ulang dilakukan pada tanggal {{ $pengumuman }} (minimal 50% dari total biaya)</i><br>
    <i>*Pelunasan maksimal tanggal {{ $pengumuman }}</i><br>
    <i>*Pembayaran SPP Juli {{ $tp_awal }} dibayarkan pada tanggal 5 - 15 Juni {{ $tp_awal }}</i>
  </p>
@endif
        <br>
