...
Lampiran 3 : <b>Pembiayaan Reguler 3</b>
...
<h5>A. Pembiayaan Peserta Didik Baru TA {{ $tp_awal }} - {{ $tp_akhir }}</h5>
    <table class="rincian">
        ...
        <tr>
          <td width="5%"> {{ $k+1 }} </td>
          <td width="50%"> {{ $b }} </td>
          <td align="right"> {{ number_format($biaya3[$b]) }} </td>
        </tr>
        @endforeach
        <tr>
          <td width="5%"> 5 </td>
        ...
        <tr>
          ...
          <td align="right"> <strong>{{ number_format($total3+$ctg->infaq+$ctg->infaqnfpeduli) }}</strong> </td>
        </tr>
      </table>
      <br>
      <table>
        <tr>
          ...
          SIT Nurul Fikri<br><h3><strong>Rp. {{ number_format($totalAll[3]+$total3+$ctg->infaq+$ctg->infaqnfpeduli) }}</strong></h3></td>
        </tr>
      </table>
</td>
<td valign="top" >
...
@foreach($kelass as $k=>$b)
        ...
        <tr>
          <td width="75%">{{ $kelas[$b->name]['ket3'] }} @ Rp. {{ number_format($kelas[$b->name]['spp3']) }} <br></td>
          <td align="right">Rp. {{ number_format($kelas[$b->name]['total3']) }}</td>
        </tr>
        @if(isset($kelas[$b->name]['daul3']))
        <tr>
          <td> Dana Tahunan</td>
          <td align="right">Rp. {{ number_format($kelas[$b->name]['daul3']) }}</td>
        </tr>
        @endif
@endforeach
</table>
...
<br>
        ...
        <table align="center" width="100%" style="font-size: 15px;">
            <tr>
                <td colspan="2" align="center"><u> Depok, {{ date("d F Y", strtotime($ctg->created_at)) }} </u><br><br></td>
                <td></td>
                <td></td>
            </tr>
            <tr>
